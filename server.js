...
db.serialize(() => {
    ...
    // Índices para Performance (Melhoria de Banco de Dados)
    ...
    db.run("CREATE INDEX IF NOT EXISTS idx_logs_line ON logs(line)");
    ...
    // Índices para Parada de Linha
    db.run("CREATE INDEX IF NOT EXISTS idx_linestops_date ON line_stops(date)");
    db.run("CREATE INDEX IF NOT EXISTS idx_linestops_status ON line_stops(status)");

    // --- MIGRATIONS FOR LINE_STOPS (CRITICAL FIX) ---
    // Ensure all columns exist even if table was created in previous version
    db.run("ALTER TABLE line_stops ADD COLUMN user_role TEXT", (err) => {});
    db.run("ALTER TABLE line_stops ADD COLUMN status TEXT", (err) => {});
    db.run("ALTER TABLE line_stops ADD COLUMN user_id TEXT", (err) => {}); 
    db.run("ALTER TABLE line_stops ADD COLUMN user_name TEXT", (err) => {});
    // -----------------------------------------------
    ...
    db.get("SELECT matricula FROM users WHERE matricula = 'admin'", (err, row) => {
        if (!row) {
            db.run(`INSERT INTO users (matricula, name, role, shift, email, password, is_admin) VALUES (?, ?, ?, ?, ?, ?, ?)`,
                ['admin', 'Admin Local', 'Admin', '1', 'admin@local.com', 'admin', 1]);
    ...
    db.get("SELECT count(*) as count FROM config_roles", (err, row) => {
        if (row && row.count === 0) {
            const defaults = [
                'Admin', 'Diretor', 'Supervisor', 'Coordenador', 'Técnico de processo', 
...
app.get('/api/logs', (req, res) => {
    // ORDER BY com índice é muito mais rápido
...
// --- LINE STOPS (PARADAS DE LINHA) ---

app.get('/api/line-stops', (req, res) => {
    db.all("SELECT * FROM line_stops ORDER BY date DESC LIMIT 500", [], (err, rows) => {
        if (err) return res.status(500).json({ error: err.message });
        const stops = rows.map(r => ({
            ...
            date: r.date,
            status: r.status,
            type: 'LINE_STOP', // Force type
            data: JSON.parse(r.data),
            itemsCount: 0,
            ngCount: 0,
            observation: '',
            signedDocUrl: JSON.parse(r.data).signedDocUrl 
        }));
        res.json(stops);
    });
});

app.post('/api/line-stops', (req, res) => {
    const { id, userId, userName, userRole, line, date, status, data, signedDocUrl } = req.body;
    
    if (!data) return res.status(400).json({error: "Dados inválidos: campo 'data' é obrigatório"});

    // Inject signedDocUrl into data blob if exists to keep it consistent
    if(signedDocUrl) {
        data.signedDocUrl = signedDocUrl;
    }

    const dataStr = JSON.stringify(data);

    db.get("SELECT id FROM line_stops WHERE id = ?", [id], (err, row) => {
        if (row) {
            db.run(`UPDATE line_stops SET line=?, status=?, data=? WHERE id=?`,
               [line, status, dataStr, id],
               function(err) {
                   if (err) {
                       console.error("Erro UPDATE line_stops:", err);
                       return res.status(500).json({ error: err.message });
                   }
                   res.json({ message: "Parada atualizada" });
               }
            );
        } else {
            db.run(`INSERT INTO line_stops (id, user_id, user_name, user_role, line, date, status, data) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
                [id, userId, userName, userRole, line, date, status, dataStr],
                function(err) {
                    if (err) {
                        console.error("Erro INSERT line_stops:", err);
                        return res.status(500).json({ error: err.message });
                    }
                    res.json({ message: "Parada registrada" });
                }
            );
        }
    });
...
app.post('/api/config/items', (req, res) => {
    ...
    // Transação para garantir atomicidade no salvamento de configurações
    db.serialize(() => {
        ...
        try {
            items.forEach(i => {
                stmt.run(i.id, i.category, i.text, i.evidence || '', i.imageUrl || '', i.type || 'LEADER');
            });
            stmt.finalize();
            db.run("COMMIT", (err) => {
                if(err) throw err;
                res.json({message: "Itens salvos com sucesso"});
            });
        } catch (e) {
            db.run("ROLLBACK");
            console.error("Erro ao salvar itens (Rollback):", e);
