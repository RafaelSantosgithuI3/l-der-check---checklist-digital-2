...
const App = () => {
  ...
  // Load Meetings when entering history
  useEffect(() => {
  ...
  const handleMaintenanceQrPhoto = async (file: File) => {
      ...
      try {
          ...
          handleMaintenanceCode(decodedText as string);
      } catch (err) {
  ...
  if (view === 'LOGIN') return <Layout variant="auth"><div className="flex flex-col items-center justify-center min-h-screen px-4"><div className="bg-zinc-900/80 backdrop-blur-xl border border-zinc-800 rounded-2xl p-8 shadow-2xl w-full max-w-md"><div className="flex justify-center mb-6"><div className="w-16 h-16 bg-gradient-to-br from-blue-600 to-indigo-700 rounded-2xl flex items-center justify-center text-white text-2xl font-bold shadow-lg shadow-blue-900/40">LC</div></div><h1 className="text-2xl font-bold text-center mb-1 text-white">Líder Check</h1><form onSubmit={handleLogin} className="space-y-4"><Input label="Matrícula" value={loginMatricula} onChange={e => setLoginMatricula(e.target.value)} icon={<UserIcon size={18} />} autoFocus /><div><label className="block text-xs font-medium text-zinc-400 mb-1.5 uppercase tracking-wide">Senha</label><div className="relative"><div className="absolute left-3 top-1/2 -translate-y-1/2 text-zinc-500 pointer-events-none"><Lock size={18} /></div><input type={showLoginPassword?"text":"password"} className="w-full pl-10 pr-10 py-2.5 bg-zinc-950 border border-zinc-800 rounded-lg text-zinc-100" value={loginPassword} onChange={e => setLoginPassword(e.target.value)} /><button type="button" onClick={() => setShowLoginPassword(!showLoginPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-zinc-500">{showLoginPassword?<EyeOff size={16}/>:<Eye size={16}/>}</button></div></div>{loginError && <div className="text-red-400 text-sm bg-red-900/20 p-3 rounded">{loginError}</div>}<Button fullWidth type="submit" disabled={isLoading}>{isLoading?'Entrando...':'Entrar'}</Button></form><div className="mt-6 flex flex-col gap-3"><button onClick={() => setView('REGISTER')} className="text-sm text-zinc-500">Cadastre-se</button><button onClick={() => setView('SETUP')} className="text-xs text-zinc-700 flex items-center justify-center gap-1 w-full"><Wifi size={12}/> Configurar Servidor</button></div></div></div></Layout>;
  if (view === 'REGISTER') return <Layout variant="auth"><div className="flex flex-col items-center justify-center min-h-screen px-4"><div className="bg-zinc-900/80 backdrop-blur-xl border border-zinc-800 rounded-2xl p-8 shadow-2xl w-full max-w-md"><h1 className="text-2xl font-bold text-center mb-1 text-white">Criar Conta</h1><form onSubmit={handleRegister} className="space-y-4"><Input label="Nome" value={regName} onChange={e => setRegName(e.target.value)} /><Input label="Matrícula" value={regMatricula} onChange={e => setRegMatricula(e.target.value)} /><select className="w-full bg-zinc-950 border border-zinc-800 rounded-lg p-2.5 text-white" value={regRole} onChange={e => setRegRole(e.target.value)}>{availableRoles.map(r => <option key={r} value={r}>{r}</option>)}</select><select className="w-full bg-zinc-950 border border-zinc-800 rounded-lg p-2.5 text-white" value={regShift} onChange={e => setRegShift(e.target.value)}><option value="1">1º Turno</option><option value="2">2º Turno</option></select><Input label="Senha" value={regPassword} onChange={e => setRegPassword(e.target.value)} type="password" /><Input label="Confirmar" value={regConfirmPassword} onChange={e => setRegConfirmPassword(e.target.value)} type="password" />{regError && <div className="text-red-400 text-sm">{regError}</div>}<Button fullWidth type="submit" disabled={isLoading}>Criar Conta</Button></form><button onClick={() => setView('LOGIN')} className="mt-4 w-full text-sm text-zinc-500">Voltar</button></div></div></Layout>;
  ...
  return (
    <Layout sidebar={<SidebarContent />}>
        ...
        {view === 'AUDIT_MENU' && (
            <>
                <header className="flex flex-col gap-4 mb-8 pb-6 border-b border-zinc-800"><h1 className="text-2xl font-bold text-zinc-100 flex items-center gap-2"><Search className="text-yellow-500" /> Auditoria</h1>
                <div className="flex gap-2 overflow-x-auto pb-2">
                    <Button variant={auditTab==='LEADER_HISTORY'?'primary':'secondary'} onClick={()=>setAuditTab('LEADER_HISTORY')}>Histórico</Button>
                    <Button variant={auditTab==='LINES'?'primary':'secondary'} onClick={()=>setAuditTab('LINES')}>Linhas</Button>
                    <Button variant={auditTab==='LEADERS'?'primary':'secondary'} onClick={()=>setAuditTab('LEADERS')}>Líderes</Button>
                    {isSuperAdmin && <Button variant={auditTab==='LEADER_EDITOR'?'primary':'secondary'} onClick={()=>setAuditTab('LEADER_EDITOR')}>Editor</Button>}
                </div>
                </header>
                {(auditTab === 'LEADER_HISTORY' || auditTab === 'MAINTENANCE_HISTORY') && (
                    <Card><div className="flex gap-4 mb-4"><input type="date" className="bg-zinc-950 border border-zinc-800 rounded p-2 text-white" value={historyDateFilter} onChange={e => setHistoryDateFilter(e.target.value)} /><select className="bg-zinc-950 border border-zinc-800 rounded p-2 text-white" value={historyShiftFilter} onChange={e => setHistoryShiftFilter(e.target.value)}><option value="ALL">Todos Turnos</option><option value="1">1º Turno</option><option value="2">2º Turno</option></select></div>
                    {historyLogs.map(log => (<div key={log.id} className="bg-zinc-950 p-4 mb-2 rounded flex justify-between items-center border border-zinc-800"><div><p className="font-bold text-white">{log.line} - {log.userName}</p><p className="text-xs text-zinc-500">{new Date(log.date).toLocaleString()} | {log.ngCount} NGs</p></div><Button size="sm" onClick={() => handleOpenPreview(log.id)}><Eye size={16}/></Button></div>))}
                    </Card>
                )}
                {auditTab === 'LINES' && (
                    <Card><div className="flex gap-4 mb-6"><input type="week" className="bg-zinc-950 border border-zinc-800 rounded p-2 text-white" value={linesWeekFilter} onChange={e=>setLinesWeekFilter(e.target.value)} /><select className="bg-zinc-950 border border-zinc-800 rounded p-2 text-white" value={linesShiftFilter} onChange={e=>setLinesShiftFilter(e.target.value)}><option value="1">1º Turno</option><option value="2">2º Turno</option></select></div>
                    <div className="overflow-x-auto"><table className="w-full text-xs"><thead className="bg-zinc-950 text-zinc-400"><tr><th className="p-2 text-left">Linha</th>{['Seg','Ter','Qua','Qui','Sex','Sab'].map(d=><th key={d} className="p-2">{d}</th>)}<th className="p-2">Ação</th></tr></thead><tbody>{linesMatrix.map(r=>(<tr key={r.line} className="border-b border-zinc-800"><td className="p-2 font-bold">{r.line}</td>{r.statuses.map((s,i)=><td key={i} className="p-2 text-center"><div className={`w-6 h-6 rounded-full flex items-center justify-center mx-auto text-[10px] ${s.status==='OK'?'bg-green-900 text-green-400':s.status==='NG'?'bg-red-900 text-red-400':'bg-zinc-800 text-zinc-500'}`}>{s.status==='OK'?'✓':s.status==='NG'?'!':'-'}</div></td>)}<td className="p-2"><Button size="sm" onClick={()=>downloadShiftExcel(r.line, linesShiftFilter, linesWeekFilter, items)}><Download size={14}/></Button></td></tr>))}</tbody></table></div>
                    </Card>
                )}
                {auditTab === 'LEADERS' && (
                    <Card><div className="flex gap-4 mb-6"><input type="week" className="bg-zinc-950 border border-zinc-800 rounded p-2 text-white" value={linesWeekFilter} onChange={e=>setLinesWeekFilter(e.target.value)} /><select className="bg-zinc-950 border border-zinc-800 rounded p-2 text-white" value={linesShiftFilter} onChange={e=>setLinesShiftFilter(e.target.value)}><option value="1">1º Turno</option><option value="2">2º Turno</option></select></div>
                    <div className="overflow-x-auto"><table className="w-full text-xs"><thead className="bg-zinc-950 text-zinc-400"><tr><th className="p-2 text-left">Líder</th>{['Seg','Ter','Qua','Qui','Sex','Sab'].map(d=><th key={d} className="p-2">{d}</th>)}</tr></thead><tbody>{leadersMatrix.map(r=>(<tr key={r.user.matricula} className="border-b border-zinc-800"><td className="p-2 font-bold">{r.user.name}</td>{r.statuses.map((s,i)=><td key={i} className="p-2 text-center"><div className={`w-6 h-6 rounded-full flex items-center justify-center mx-auto text-[10px] ${s.status==='OK'?'bg-green-900 text-green-400':s.status==='NG'?'bg-red-900 text-red-400':'bg-zinc-800 text-zinc-500'}`}>{s.status==='OK'?'✓':s.status==='NG'?'!':'-'}</div></td>)}</tr>))}</tbody></table></div></Card>
                )}
                {auditTab === 'LEADER_EDITOR' && (
                    <Card><div className="flex justify-between mb-4"><h3 className="font-bold">Editor de Itens</h3><Button onClick={()=>handleSaveEditor(leaderItems, 'LEADER')}><Save size={16}/> Salvar</Button></div>
                    {leaderItems.map(item => (<div key={item.id} className="bg-zinc-950 p-3 mb-2 rounded border border-zinc-800 flex gap-2"><div className="flex-1"><label className="text-[10px] text-zinc-500">Categoria</label><input className="w-full bg-zinc-900 border border-zinc-800 rounded p-1 text-white" value={item.category} onChange={e=>handleEditorChange(leaderItems,setLeaderItems,item.id,'category',e.target.value)}/></div><div className="flex-[3]"><label className="text-[10px] text-zinc-500">Item</label><input className="w-full bg-zinc-900 border border-zinc-800 rounded p-1 text-white" value={item.text} onChange={e=>handleEditorChange(leaderItems,setLeaderItems,item.id,'text',e.target.value)}/></div><div className="flex items-end gap-2"><label className="cursor-pointer bg-zinc-800 p-2 rounded text-zinc-400"><ImageIcon size={16}/><input type="file" className="hidden" onChange={e=>{if(e.target.files?.[0])handleEditorImage(leaderItems,setLeaderItems,item.id,e.target.files[0])}}/></label><button onClick={()=>handleEditorDelete(leaderItems,setLeaderItems,item.id)} className="text-red-500 p-2"><Trash2 size={16}/></button></div></div>))}
                    <Button fullWidth variant="outline" onClick={()=>handleEditorAdd(leaderItems,setLeaderItems,'LEADER')}><Plus size={16}/> Novo Item</Button></Card>
                )}
            </>
        )}
        ...
        {view === 'MANAGEMENT' && (
            <>
            <header className="flex items-center justify-between mb-8 pb-6 border-b border-zinc-800"><h1 className="text-2xl font-bold text-zinc-100 flex items-center gap-2"><Briefcase className="text-cyan-500" /> Gestão Centralizada</h1></header>
            <div className="flex gap-2 mb-6 overflow-x-auto pb-2"><Button variant={managementTab==='LINES'?'primary':'secondary'} onClick={()=>setManagementTab('LINES')}>Linhas</Button><Button variant={managementTab==='ROLES'?'primary':'secondary'} onClick={()=>setManagementTab('ROLES')}>Cargos</Button><Button variant={managementTab==='MODELS'?'primary':'secondary'} onClick={()=>setManagementTab('MODELS')}>Modelos</Button><Button variant={managementTab==='STATIONS'?'primary':'secondary'} onClick={()=>setManagementTab('STATIONS')}>Postos</Button></div>
            {managementTab === 'LINES' && <Card><div className="flex gap-2 mb-4"><Input value={newItemName} onChange={e=>setNewItemName(e.target.value)} placeholder="Nova Linha"/><Button onClick={()=>handleAddItem(lines, setLines, saveLines)}>Add</Button></div><div className="grid grid-cols-2 gap-2">{lines.map(l=><div key={l} className="bg-zinc-950 p-2 rounded border border-zinc-800 flex justify-between">{l}<button onClick={()=>handleDeleteItem(l,lines,setLines,saveLines)} className="text-red-500"><X size={14}/></button></div>)}</div></Card>}
            {managementTab === 'ROLES' && <Card><div className="flex gap-2 mb-4"><Input value={newItemName} onChange={e=>setNewItemName(e.target.value)} placeholder="Novo Cargo"/><Button onClick={()=>handleAddItem(availableRoles, setAvailableRoles, saveRoles)}>Add</Button></div><div className="grid grid-cols-2 gap-2">{availableRoles.map(r=><div key={r} className="bg-zinc-950 p-2 rounded border border-zinc-800 flex justify-between">{r}<button onClick={()=>handleDeleteItem(r,availableRoles,setAvailableRoles,saveRoles)} className="text-red-500"><X size={14}/></button></div>)}</div></Card>}
            {managementTab === 'MODELS' && <Card><div className="flex gap-2 mb-4"><Input value={newItemName} onChange={e=>setNewItemName(e.target.value)} placeholder="Novo Modelo"/><Button onClick={()=>handleAddItem(models, setModels, saveModels)}>Add</Button></div><div className="grid grid-cols-2 gap-2">{models.map(m=><div key={m} className="bg-zinc-950 p-2 rounded border border-zinc-800 flex justify-between">{m}<button onClick={()=>handleDeleteItem(m,models,setModels,saveModels)} className="text-red-500"><X size={14}/></button></div>)}</div></Card>}
            {managementTab === 'STATIONS' && <Card><div className="flex gap-2 mb-4"><Input value={newItemName} onChange={e=>setNewItemName(e.target.value)} placeholder="Novo Posto"/><Button onClick={()=>handleAddItem(stations, setStations, saveStations)}>Add</Button></div><div className="grid grid-cols-2 gap-2">{stations.map(s=><div key={s} className="bg-zinc-950 p-2 rounded border border-zinc-800 flex justify-between">{s}<button onClick={()=>handleDeleteItem(s,stations,setStations,saveStations)} className="text-red-500"><X size={14}/></button></div>)}</div></Card>}
            </>
        )}

        {view === 'LINE_STOP_DASHBOARD' && (
            <>
            <header className="flex flex-col gap-4 mb-8 pb-6 border-b border-zinc-800"><h1 className="text-2xl font-bold text-zinc-100 flex items-center gap-2"><AlertTriangle className="text-red-500" /> Parada de Linha</h1><div className="flex gap-2 overflow-x-auto pb-2"><Button variant={lineStopTab==='NEW'?'primary':'secondary'} onClick={()=>setLineStopTab('NEW')}>Novo</Button><Button variant={lineStopTab==='PENDING'?'primary':'secondary'} onClick={()=>setLineStopTab('PENDING')}>Pendentes</Button><Button variant={lineStopTab==='HISTORY'?'primary':'secondary'} onClick={()=>setLineStopTab('HISTORY')}>Histórico</Button></div></header>
            {lineStopTab === 'NEW' && <div className="space-y-6 max-w-4xl mx-auto pb-20"><Card><div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div><label className="text-xs font-bold text-zinc-500 block uppercase">Modelo</label><input className="w-full bg-zinc-950 border border-zinc-800 rounded p-2 text-white" list="models-list" value={lineStopData.model} onChange={e=>setLineStopData({...lineStopData,model:e.target.value})} placeholder="Selecione ou digite..." /><datalist id="models-list">{models.map(m=><option key={m} value={m}/>)}</datalist></div>
                <Input label="Linha" value={lineStopData.line} onChange={e=>setLineStopData({...lineStopData,line:e.target.value})}/>
                <div><label className="text-xs font-bold text-zinc-500 block uppercase">Posto (De)</label><input className="w-full bg-zinc-950 border border-zinc-800 rounded p-2 text-white" list="stations-list" value={lineStopData.stationStart} onChange={e=>setLineStopData({...lineStopData,stationStart:e.target.value})} /><datalist id="stations-list">{stations.map(s=><option key={s} value={s}/>)}</datalist></div>
                <Input label="Motivo" value={lineStopData.motivo} onChange={e=>setLineStopData({...lineStopData,motivo:e.target.value})}/>
                <Input type="time" label="Início" value={lineStopData.startTime} onChange={e=>setLineStopData({...lineStopData,startTime:e.target.value})}/>
                </div>
                <div className="mt-4"><label className="text-xs font-bold text-zinc-500 uppercase block mb-2">Setor Responsável</label><div className="grid grid-cols-2 md:grid-cols-5 gap-2">{SECTORS_LIST.map(s=><button key={s} onClick={()=>setLineStopData({...lineStopData,responsibleSector:s})} className={`p-2 rounded text-xs border ${lineStopData.responsibleSector===s?'bg-blue-600 border-blue-500 text-white':'bg-zinc-950 border-zinc-800 text-zinc-400'}`}>{s}</button>)}</div></div>
                <Button fullWidth className="mt-6" onClick={handleSaveLineStop}>Salvar Reporte</Button></Card></div>}
            {lineStopTab === 'PENDING' && <div className="space-y-4">{lineStopLogs.filter(l=>l.status==='WAITING_JUSTIFICATION').map(log=><div key={log.id} className="bg-zinc-900 border border-zinc-800 p-4 rounded flex justify-between"><div><p className="font-bold text-white">{(log.data as any).model} - {log.line}</p><p className="text-sm text-zinc-400">{(log.data as any).motivo}</p></div><Button onClick={()=>{setActiveLineStopLog(log);setJustificationInput('');}}>Justificar</Button></div>)}
            {activeLineStopLog && <div className="mt-4 bg-zinc-950 p-4 border border-zinc-800 rounded"><Input value={justificationInput} onChange={e=>setJustificationInput(e.target.value)} placeholder="Justificativa"/><div className="flex gap-2 mt-2"><Button onClick={handleSaveJustification}>Salvar</Button><Button variant="secondary" onClick={()=>setActiveLineStopLog(null)}>Cancelar</Button></div></div>}</div>}
            {lineStopTab === 'HISTORY' && <div className="space-y-2">{lineStopLogs.filter(l=>l.status!=='WAITING_JUSTIFICATION').map(l=><div key={l.id} className="bg-zinc-950 p-3 border border-zinc-800 rounded flex justify-between"><span className="text-white font-bold">{l.line}</span><Button size="sm" variant="secondary" onClick={()=>handleOpenPreview(l.id)}><Eye size={14}/></Button></div>)}</div>}
            </>
        )}

        {view === 'MEETING_MENU' && (
             <div className="grid grid-cols-1 md:grid-cols-2 gap-6"><div onClick={() => setView('MEETING_FORM')} className="group bg-zinc-900 p-6 rounded-2xl border border-zinc-800 hover:border-emerald-600 cursor-pointer"><div className="w-12 h-12 bg-emerald-600/20 text-emerald-500 rounded-xl flex items-center justify-center mb-4"><Plus size={24} /></div><h3 className="font-bold text-xl text-zinc-100">Nova Ata</h3></div><div onClick={() => setView('MEETING_HISTORY')} className="group bg-zinc-900 p-6 rounded-2xl border border-zinc-800 hover:border-blue-600 cursor-pointer"><div className="w-12 h-12 bg-blue-600/20 text-blue-500 rounded-xl flex items-center justify-center mb-4"><History size={24} /></div><h3 className="font-bold text-xl text-zinc-100">Histórico</h3></div></div>
        )}

        {view === 'MEETING_FORM' && (
            <div className="space-y-6 max-w-3xl mx-auto"><Card><Input label="Título" value={meetingTitle} onChange={e => setMeetingTitle(e.target.value)} /><div className="flex gap-4 mt-4"><Input type="time" label="Início" value={meetingStartTime} onChange={e => setMeetingStartTime(e.target.value)} /><Input type="time" label="Fim" value={meetingEndTime} onChange={e => setMeetingEndTime(e.target.value)} /></div></Card><Card><h3 className="text-xs font-bold text-zinc-400 uppercase mb-3">Foto</h3><label className="cursor-pointer bg-zinc-950 p-8 border-2 border-dashed border-zinc-800 rounded flex justify-center"><Camera className="text-zinc-600"/><input type="file" className="hidden" onChange={e=>{if(e.target.files?.[0])handleMeetingPhoto(e.target.files[0])}}/></label></Card><Card><div className="flex gap-2 mb-4"><Input placeholder="Nome" value={newParticipant} onChange={e => setNewParticipant(e.target.value)} list="users-list"/><datalist id="users-list">{usersList.map(u=><option key={u.matricula} value={u.name}/>)}</datalist><Button onClick={handleAddParticipant}>+</Button></div><div className="flex flex-wrap gap-2">{meetingParticipants.map((p,i)=><div key={i} className="bg-zinc-800 px-3 py-1 rounded-full text-sm flex gap-2">{p}<button onClick={()=>handleRemoveParticipant(i)} className="text-red-400">x</button></div>)}</div></Card><Card><textarea className="w-full bg-zinc-950 border border-zinc-800 rounded p-4 text-white h-32" placeholder="Pauta" value={meetingTopics} onChange={e => setMeetingTopics(e.target.value)}/></Card><Button fullWidth onClick={handleSaveMeeting}>Salvar Ata</Button></div>
        )}

        {view === 'MEETING_HISTORY' && (
            <div className="space-y-4">{meetingHistory.map(m => (<div key={m.id} className="bg-zinc-900 border border-zinc-800 rounded-xl p-5 flex justify-between items-center"><div><p className="font-bold text-white">{m.title}</p><p className="text-sm text-zinc-400">{new Date(m.date).toLocaleDateString()}</p></div><div className="flex gap-2"><Button size="sm" variant="secondary" onClick={() => setPreviewMeeting(m)}><Eye size={16}/></Button><Button size="sm" variant="outline" onClick={() => exportMeetingToExcel(m)}><Download size={16}/></Button></div></div>))}</div>
        )}

        {view === 'DASHBOARD' && (
            <>
            <div className="flex justify-between items-center mb-6"><div><h1 className="text-2xl font-bold text-white">{currentLine}</h1><p className="text-zinc-400">{getManausDate().toLocaleDateString()}</p></div><Button variant="outline" onClick={()=>setView('MENU')}>Voltar</Button></div>
            <div className="grid md:grid-cols-4 gap-8"><div className="hidden md:block"><div className="sticky top-8 space-y-2">{categories.map(c=><button key={c} onClick={()=>categoryRefs.current[c]?.scrollIntoView({behavior:'smooth'})} className="block text-zinc-400 hover:text-white text-sm text-left">{c}</button>)}</div></div>
            <div className="md:col-span-3 space-y-8">{categories.map(cat=><div key={cat} ref={el=>{categoryRefs.current[cat]=el}}><h2 className="text-lg font-bold text-white mb-4 border-l-4 border-blue-600 pl-3">{cat}</h2>{items.filter(i=>i.category===cat).map(item=>(<div key={item.id} className="bg-zinc-900 border border-zinc-800 p-4 rounded-xl mb-4"><p className="text-zinc-200 font-medium mb-2">{item.text}</p>{item.imageUrl && <img src={item.imageUrl} className="h-32 mb-4 rounded border border-zinc-700"/>}<div className="flex gap-2"><button onClick={()=>setChecklistData({...checklistData,[item.id]:'OK'})} className={`flex-1 py-2 rounded font-bold border ${checklistData[item.id]==='OK'?'bg-green-900/30 border-green-500 text-green-400':'bg-zinc-950 border-zinc-800 text-zinc-500'}`}>OK</button><button onClick={()=>setChecklistData({...checklistData,[item.id]:'NG'})} className={`flex-1 py-2 rounded font-bold border ${checklistData[item.id]==='NG'?'bg-red-900/30 border-red-500 text-red-400':'bg-zinc-950 border-zinc-800 text-zinc-500'}`}>NG</button><button onClick={()=>setChecklistData({...checklistData,[item.id]:'N/A'})} className={`w-16 py-2 rounded font-bold border ${checklistData[item.id]==='N/A'?'bg-yellow-900/30 border-yellow-500 text-yellow-400':'bg-zinc-950 border-zinc-800 text-zinc-500'}`}>N/A</button></div>{checklistData[item.id]==='NG' && <div className="mt-3 p-3 bg-red-900/10 border border-red-900/30 rounded"><Input placeholder="Comentário" value={checklistEvidence[item.id]?.comment||''} onChange={e=>handleNgComment(item.id,e.target.value)} className="mb-2"/><label className="cursor-pointer text-xs bg-zinc-800 px-3 py-2 rounded inline-flex items-center gap-2"><Camera size={14}/> Foto<input type="file" className="hidden" onChange={e=>{if(e.target.files?.[0])handleNgPhoto(item.id,e.target.files[0])}}/></label></div>}</div>))}</div>)}</div></div>
            <div className="fixed bottom-0 right-0 left-0 p-4 bg-zinc-950 border-t border-zinc-800 flex justify-end"><Button onClick={async()=>{if(!currentUser)return; setIsLoading(true); await saveLog({id:Date.now().toString(),userId:currentUser.matricula,userName:currentUser.name,userRole:currentUser.role,line:currentLine,date:getManausDate().toISOString(),itemsCount:items.length,ngCount:Object.values(checklistData).filter(v=>v==='NG').length,observation,data:checklistData,evidenceData:checklistEvidence,type:isMaintenanceMode?'MAINTENANCE':'PRODUCTION',maintenanceTarget}); setIsLoading(false); setView('SUCCESS');}} className="shadow-lg shadow-blue-900/20"><Save size={18}/> Finalizar</Button></div>
            </>
        )}

        {view === 'MAINTENANCE_QR' && <div className="max-w-md mx-auto text-center"><Card><label className="cursor-pointer block p-10 border-2 border-dashed border-zinc-700 rounded mb-6"><Camera className="mx-auto mb-2 text-zinc-500"/><span className="text-zinc-400">Ler QR Code (Arquivo/Foto)</span><input type="file" accept="image/*" className="hidden" onChange={e=>{if(e.target.files?.[0])handleMaintenanceQrPhoto(e.target.files[0])}}/></label><div className="flex gap-2"><Input placeholder="Código Manual" value={qrCodeManual} onChange={e=>setQrCodeManual(e.target.value)}/><Button onClick={()=>handleMaintenanceCode(qrCodeManual)}>Ir</Button></div><div id="reader-hidden" className="hidden"></div></Card></div>}
        
        {view === 'CHECKLIST_MENU' && showLinePrompt && (
             <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50"><Card className="w-full max-w-sm"><h3 className="font-bold text-white mb-4">Selecionar Linha</h3><select className="w-full bg-zinc-950 border border-zinc-800 rounded p-3 text-white mb-4" value={currentLine} onChange={e=>setCurrentLine(e.target.value)}>{lines.map(l=><option key={l} value={l}>{l}</option>)}</select><Button fullWidth onClick={handleConfirmLine}>Confirmar</Button></Card></div>
        )}

        {view === 'SUCCESS' && (
            <div className="flex flex-col items-center justify-center min-h-screen text-center"><div className="w-24 h-24 bg-green-500/10 text-green-500 rounded-full flex items-center justify-center mb-6 border border-green-500/20 shadow-[0_0_30px_rgba(34,197,94,0.2)] animate-in zoom-in duration-300"><CheckCircle2 size={48} /></div><h2 className="text-3xl font-bold text-white mb-2">Salvo com Sucesso!</h2><p className="text-zinc-400 mb-8 max-w-md">Os dados foram registrados no sistema.</p><Button onClick={() => setView('MENU')} className="min-w-[200px]">Voltar ao Início</Button></div>
        )}

        {view === 'PERSONAL' && (
            <div className="space-y-4"><header className="flex items-center justify-between mb-8"><h1 className="text-2xl font-bold text-white">Meus Registros</h1></header>{personalLogs.length === 0 && <p className="text-zinc-500 text-center py-12 bg-zinc-900/50 rounded-xl border border-zinc-800">Nenhum registro encontrado.</p>}{personalLogs.map(log => (<div key={log.id} className="bg-zinc-900 border border-zinc-800 rounded-xl p-5 flex flex-col md:flex-row justify-between items-center gap-4 hover:border-zinc-700 transition-colors"><div className="flex items-center gap-4"><div className={`w-10 h-10 rounded-full flex items-center justify-center text-lg font-bold ${log.ngCount > 0 ? 'bg-red-900/20 text-red-500 border border-red-900/30' : 'bg-green-900/20 text-green-500 border border-green-900/30'}`}>{log.ngCount > 0 ? '!' : '✓'}</div><div><p className="font-bold text-zinc-200">{new Date(log.date).toLocaleString()}</p><p className="text-sm text-zinc-400">{log.line} <span className="mx-2">•</span> {log.ngCount > 0 ? `${log.ngCount} Falhas` : '100% OK'} {log.type === 'LINE_STOP' && '(Parada)'}</p></div></div><div className="flex gap-2 w-full md:w-auto"><Button variant="secondary" onClick={() => handleOpenPreview(log.id)} className="flex-1 md:flex-none"><Eye size={16}/></Button><Button variant="outline" onClick={() => exportLogToExcel(log, items)} className="flex-1 md:flex-none"><Download size={16}/> Excel</Button></div></div>))}</div>
        )}

        {view === 'PROFILE' && (
            <div className="max-w-xl mx-auto"><header className="flex items-center justify-between mb-8"><h1 className="text-2xl font-bold text-white">Meu Perfil</h1></header><Card><div className="flex flex-col items-center mb-8"><div className="w-24 h-24 bg-zinc-800 rounded-full flex items-center justify-center text-3xl font-bold mb-4 text-zinc-300 border-2 border-zinc-700 shadow-xl">{profileData?.name.charAt(0)}</div><p className="text-xl font-bold text-white">{profileData?.name}</p><p className="text-zinc-500 bg-zinc-950 px-3 py-1 rounded-full text-xs mt-2 border border-zinc-800">{profileData?.role}</p></div><div className="space-y-5"><Input label="Nome" value={profileData?.name} onChange={e => setProfileData({...profileData!, name: e.target.value})} /><Input label="Email" value={profileData?.email} onChange={e => setProfileData({...profileData!, email: e.target.value})} /><Input label="Alterar Senha" type="password" placeholder="Nova senha (opcional)" value={profileData?.password || ''} onChange={e => setProfileData({...profileData!, password: e.target.value})} /><div className="pt-4"><Button fullWidth onClick={handleSaveProfile}>Salvar Alterações</Button></div></div></Card></div>
        )}

        {view === 'ADMIN' && (
            <>
            <header className="flex items-center justify-between mb-8 pb-6 border-b border-zinc-800"><h1 className="text-2xl font-bold text-zinc-100 flex items-center gap-2"><Shield className="text-zinc-400" /> Painel Administrativo</h1></header>
            <div className="flex gap-2 mb-6 overflow-x-auto pb-2"><Button variant={adminTab === 'USERS' ? 'primary' : 'secondary'} onClick={() => setAdminTab('USERS')}>Usuários</Button><Button variant={adminTab === 'PERMISSIONS' ? 'primary' : 'secondary'} onClick={() => setAdminTab('PERMISSIONS')}>Permissões</Button></div>
            {adminTab === 'PERMISSIONS' && <Card className="overflow-x-auto"><table className="w-full text-sm text-center border-collapse"><thead><tr className="bg-zinc-950 text-zinc-400"><th className="p-3 text-left">Cargo</th>{['CHECKLIST', 'LINE_STOP', 'MEETING', 'MAINTENANCE', 'AUDIT', 'ADMIN', 'MANAGEMENT'].map(mod => (<th key={mod} className="p-3 min-w-[100px] text-xs uppercase">{MODULE_NAMES[mod] || mod}</th>))}</tr></thead><tbody className="divide-y divide-zinc-800">{availableRoles.map(role => (<tr key={role} className="hover:bg-zinc-900"><td className="p-3 text-left font-bold text-white">{role}</td>{['CHECKLIST', 'LINE_STOP', 'MEETING', 'MAINTENANCE', 'AUDIT', 'ADMIN', 'MANAGEMENT'].map((module: any) => { const perm = permissions.find(p => p.role === role && p.module === module); const isAllowed = perm ? perm.allowed : (['CHECKLIST','MEETING','MAINTENANCE','LINE_STOP'].includes(module)); return (<td key={module} className="p-3"><input type="checkbox" checked={isAllowed} onChange={() => handleTogglePermission(role, module)} className="w-4 h-4 rounded border-zinc-700 bg-zinc-900 text-blue-600 focus:ring-blue-600/50" /></td>) })}</tr>))}</tbody></table></Card>}
            {adminTab === 'USERS' && <Card><div className="flex justify-between items-center mb-4"><h3 className="text-lg font-bold">Usuários</h3><Button onClick={() => setView('REGISTER')} variant="outline" size="sm"><UserPlus size={16}/> Novo</Button></div><div className="overflow-x-auto"><table className="w-full text-sm text-left text-zinc-300"><thead className="text-xs text-zinc-400 uppercase bg-zinc-950"><tr><th>Nome</th><th>Matrícula</th><th>Função</th><th>Admin</th><th>Ações</th></tr></thead><tbody className="divide-y divide-zinc-800">{usersList.map(u => (<tr key={u.matricula}><td className="px-4 py-3">{u.name}</td><td className="px-4 py-3">{u.matricula}</td><td className="px-4 py-3">{u.role}</td><td className="px-4 py-3">{u.isAdmin ? <span className="text-green-400 font-bold bg-green-900/30 px-2 py-1 rounded text-xs border border-green-900/50">ADMIN</span> : <span className="text-zinc-600">-</span>}</td><td className="px-4 py-3"><button onClick={() => openEditModal(u)} className="mr-2 text-blue-400"><Edit3 size={16}/></button><button onClick={async () => { if(confirm('Excluir?')) { await deleteUser(u.matricula); setUsersList(await getAllUsers()); }}} className="text-red-400"><Trash2 size={16}/></button></td></tr>))}</tbody></table></div></Card>}
            {showUserEditModal && editingUser && <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[100] flex items-center justify-center p-4"><Card className="w-full max-w-md bg-zinc-900 border-zinc-800"><h3 className="text-xl font-bold mb-4">Editar Usuário</h3><div className="space-y-3"><Input label="Nome" value={editingUser.name} onChange={e => setEditingUser({...editingUser, name: e.target.value})} /><Input label="Matrícula" value={editingUser.matricula} onChange={e => setEditingUser({...editingUser, matricula: e.target.value})} /><div><label className="text-xs font-medium text-zinc-400 mb-1">Função</label><select className="w-full bg-zinc-950 border border-zinc-800 rounded p-2 text-white" value={editingUser.role} onChange={e => setEditingUser({...editingUser, role: e.target.value})}>{availableRoles.map(r => <option key={r} value={r}>{r}</option>)}</select></div><div><label className="text-xs font-medium text-zinc-400 mb-1">Turno</label><select className="w-full bg-zinc-950 border border-zinc-800 rounded p-2 text-white" value={editingUser.shift} onChange={e => setEditingUser({...editingUser, shift: e.target.value})}><option value="1">1º Turno</option><option value="2">2º Turno</option></select></div><Input label="Nova Senha (Opcional)" value={editingUser.password || ''} onChange={e => setEditingUser({...editingUser, password: e.target.value})} /><div className="flex items-center gap-2 mt-2"><input type="checkbox" id="isAdminCheck" checked={editingUser.isAdmin || false} onChange={e => setEditingUser({...editingUser, isAdmin: e.target.checked})} /><label htmlFor="isAdminCheck" className="text-sm text-zinc-300">Acesso Admin Global</label></div><div className="flex gap-2 mt-4"><Button variant="secondary" fullWidth onClick={() => setShowUserEditModal(false)}>Cancelar</Button><Button fullWidth onClick={saveUserChanges}>Salvar</Button></div></div></Card></div>}
            </>
        )}

        {previewMeeting && (
            <div className="fixed inset-0 bg-black/90 flex items-center justify-center p-4 z-[100]"><Card className="w-full max-w-2xl max-h-[90vh] overflow-y-auto"><div className="flex justify-between mb-4"><h3 className="font-bold text-white">{previewMeeting.title}</h3><button onClick={()=>setPreviewMeeting(null)}><X/></button></div><div className="space-y-4 text-sm text-zinc-300"><p>Data: {new Date(previewMeeting.date).toLocaleDateString()} ({previewMeeting.startTime} - {previewMeeting.endTime})</p>{previewMeeting.photoUrl && <img src={previewMeeting.photoUrl} className="w-full rounded border border-zinc-700"/>}<div><h4 className="font-bold text-white mb-2">Participantes</h4><div className="flex flex-wrap gap-2">{previewMeeting.participants.map(p=><span key={p} className="bg-zinc-800 px-2 rounded">{p}</span>)}</div></div><div><h4 className="font-bold text-white mb-2">Pauta</h4><p className="bg-zinc-950 p-3 rounded whitespace-pre-wrap">{previewMeeting.topics}</p></div></div></Card></div>
        )}
        {previewLog && renderPreviewModal()}
    </Layout>
  );
};
